...
def top_articles_for_period(period_start=None, top_n: int = 3, exogenous_overrides: dict | None = None) -> dict:
    """Los `top_n` articulos con mayor `units_por_dia` previsto para un periodo,
    pensado para orientar la compra semanal (que platos van a moverse mas).

    Misma logica de modos que `predict_sales` (historical/forecast/
    forecast_sin_historial_reciente), pero para los 82 articulos a la vez en
    vez de uno solo -- se construye la tabla de features una unica vez y se
    predice sobre todas las filas del periodo de golpe.
    """
    model = _load_model()
    panel = load_sales_panel()
    ...
    period_rows = table.loc[table["report_start"] == target_start].copy()
    if period_rows.empty:
        raise ValueError(f"No hay datos suficientes para el periodo {target_start.date()}.")

    period_rows["prediction_units_por_dia"] = model.predict(period_rows[SALES_FEATURE_COLS])
    period_rows["prediction_units_periodo"] = period_rows["prediction_units_por_dia"] * PERIOD_LENGTH_DAYS
    top = period_rows.sort_values("prediction_units_por_dia", ascending=False).head(top_n)
    ...
    result = {
        ...
        "top_articles": [
            {
                "article_code": int(row["article_code"]),
                "article_name": str(row["article_name"]),
                "prediction_units_por_dia": round(float(row["prediction_units_por_dia"]), 3),
                "prediction_units_periodo": round(float(row["prediction_units_periodo"]), 1),
            }
    ...
    if not used_recent_history:
        result["aviso"] = (
            "Sin historial reciente real para este periodo -- lag_1/ewma_span3 se "
            "sustituyeron por la media historica de cada articulo. Menos fiable que "
            "una prevision del periodo inmediatamente siguiente."
        )
    return result
